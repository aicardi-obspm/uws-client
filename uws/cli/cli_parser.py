...
def build_list_argparse(subparsers):
    ...
    parser_list.add_argument('-A', '--archived', action='store_true',
                             help='[UWS1.1] show (deleted) jobs archived on the server')
    ...
    parser_list.add_argument('--last', action='store',
                             help='[UWS1.1] show only most recently started jobs')
...
def build_job_argparse(subparsers):
    ...
    job_subparsers = parser_job.add_subparsers(dest='job_command',
                                               help='commands for manipulating jobs')
    parser_job_show = job_subparsers.add_parser('show', help='show the specific job')
    parser_job_show.add_argument('id', help='job id')
    parser_job_show.add_argument('-w', '--wait', nargs='?', const='-1', default=None,
                                 help=('[UWS1.1] wait for phase change before returning, '
                                       'but at most the specified amount of seconds or '
                                       'infinitely, if no value is given'))
    ...
    parser_job_new.add_argument('-r', '--run', action='store_true',
                                help='immediately submits the job on creation')
    parser_job_new.add_argument('job_parameters', nargs='*',
                                help=('unspecified list of UWS service parameters in the form'
                                      ' "<parameter>=<value>" - Default parameters are: '
    ...
    parser_job_set.add_argument('job_parameters', nargs='*',
                                help=('unspecified list of UWS service parameters in the form'
                                      ' "<parameter>=<value>" - Default parameters are: '
                                      'destruction (Destruction time of the job), '
                                      'executionDuration (Execution duration of the job in seconds)'
    ...
    parser_job_run = job_subparsers.add_parser('run',
                                               help="run the specific job if its state is pending")
    parser_job_run.add_argument('id', help='job id')

    parser_job_abort = job_subparsers.add_parser('abort',
                                                 help="aborts the execution of a specific job")
    parser_job_abort.add_argument('id', help='job id')

    parser_job_abort = job_subparsers.add_parser('delete', help="delete a specific job")
    parser_job_abort.add_argument('id', help='job id')

    parser_job_results = job_subparsers.add_parser('results',
                                                   help="download results of a specific job")
    parser_job_results.add_argument('id', help='job id')
    parser_job_results.add_argument('result_id', nargs='?',
                                    help='result id (e.g. for specifying the format, optional)')
    parser_job_results.add_argument('-f', '--file_base',
                                    help=('basename of output file (optional), '
                                          'will be appended with result_id')
                                   )
